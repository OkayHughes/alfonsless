...
% - params.numCons:     number of SOS polynomial constraints
% - params.nxCons:      number of dimensions of x corresponding to each
%                       constraint, so sum(nxCons) = length(x)
...
% - params.Pconsidx:    1-by-numCons list of indices of the sets of points
%                       in P corresponding to each constraint
% - params.P:           evaluations of the basis for the space of 
%                       (params.n)-variate degree-(params.d) polynomials
%                       at the interpolation points ; if idx = Pconsidx{1}
%                       then P{idx} is the set of points for constraint 1
...
% -------------------------------------------------------------------------
    ...
    n = params.n ;
    U = params.U ;
    numCons = params.numCons ;
    Pconsidx = params.Pconsidx ;
    P = params.P ;
    PWts = params.PWts ;
    ...
    off = 0 ;
    for consIdx = 1:numCons
        xidx = off+(1:nxCons(consIdx)) ;
        xPoly = x(xidx) ;
        
        % for the weight 1
        Pidx = P{Pconsidx(consIdx)} ;
        [inPoly, gPoly, HPoly] = gH_SOSWt(xPoly,Pidx) ;
