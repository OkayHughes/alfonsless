...
for i=1:numPolys
    ...
    nu                      = L+sum(LWts) ;
    gH_Params.bnu	        = gH_Params.bnu + nu+1;
...
[~, g0, ~, ~] = alfonso_grad_and_hess_new(x0, gH_Params);
...
opts.optimTol = 1e-6 ;
%opts.verbose = 0;
results = alfonso(probData, x0, @alfonso_grad_and_hess_new, gH_Params, opts);
...
surf(grid_x, grid_y, alf_vals);
surf(grid_x, grid_y, spot_vals);
